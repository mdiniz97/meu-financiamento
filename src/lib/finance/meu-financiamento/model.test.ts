...
it('pagas encadeia só as parcelas: amortizações extras não entram na composição', () => {
  const movimentos = [
  ...
  // A composição (juros/correção/seguro/amortização/saldo) é idêntica com ou
  // sem extras: o saldo exibido na tabela é o do encadeamento sem extras.
  expect(comExtra.pagas).toEqual(semExtra.pagas);

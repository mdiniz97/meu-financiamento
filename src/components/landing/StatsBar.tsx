...
const count = (v: number) => Math.round(v).toLocaleString("pt-BR");
const brlMilhoes = (v: number) => `R$ ${v.toFixed(1).replace(".", ",")} mi`;
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
        <div className="grid grid-cols-1 divide-y divide-border border border-border sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
          <div className="flex flex-col items-center gap-1 p-6 text-center">
            <AnimatedNumber value={10000} format={count} className="font-mono text-2xl font-bold tabular-nums" />
          ...
          <div className="flex flex-col items-center gap-1 p-6 text-center">
            <AnimatedNumber value={99} format={pct} className="font-mono text-2xl font-bold tabular-nums" />
